...
class TestORM(TransactionCase):
    ...
    def test_access_filtered_records(self):
        ...
        partner_model = self.env['ir.model'].search([('model', '=', 'test_orm.partner')])
...
class TestRecordset(TestOrmPartnerCommon, TransactionCase):
    ...
    def test_record_cache_behavior(self):
        ...
        for p in data:
            partner_ids.append(self.env['test_orm.partner'].create({
                'name': p,
                'child_ids': [Command.create({'name': c}) for c in data[p]],
            }).id)

        partners = self.env['test_orm.partner'].search([('id', 'in', partner_ids)])
