...
class upgrade:
    def __init__(self, file_manager):
            ...
            for operation in MODES:
                ...
                for perm in perms:
                    if operation not in perm.operations or any(
                        ...
                        and rule.module in self.get_all_module_dependencies(perm.module)
                        for rule in rules
                    ):
                        continue
        ...
        for perm in (x for xs in ir_perms.values() for x in xs):
            ...
            perm_modules = self.get_all_module_dependencies(perm.module)
        ...
        for rule in (x for xs in ir_rules.values() for x in xs):
            ...
            for subgroup in group_defs.implying(rule.group):
                ...
                rule_modules = self.get_all_module_dependencies(rule.module)
                ...
                for perm in ir_perms[rule.model]:
                    if perm.group in groups:
                        ...
                        elif rule.module in self.get_all_module_dependencies(perm.module):
