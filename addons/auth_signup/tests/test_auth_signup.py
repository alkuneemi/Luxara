...
class TestAuthSignupFlow(HttpCaseWithUserPortal, HttpCaseWithUserDemo):
    ...
    def test_alert_new_device_lang(self):
        ...
        with patch.object(self.env.registry['mail.mail'], 'unlink', lambda m: None):
            with patch.object(self.env.registry['res.users'], '_should_alert_new_device', lambda u: True):
