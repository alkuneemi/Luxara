...
class L10n_Es_Edi_TbaiDocument(models.Model):
        ...
        return {
            ...
            'headers': {
                ...
                'eus-bizkaia-n3-data': json.dumps({
                    ...
                    'inte': {
                        'nif': self.env['res.partner']._split_vat(company.vat, 'ES')[1],
                        'nrs': company.name,
                    },
                    'drs': {
    ...
    def _generate_final_xml_bi(self, freelancer=False):
        ...
        lroe_values = {
            ...
            'sender': sender,
            'sender_vat': self.env['res.partner']._split_vat(sender.vat, 'ES')[1],
    ...
    def _get_recipient_values(self, partner, is_simplified=False):
        ...
        if not partner._l10n_es_is_foreign() and partner.vat:
            recipient_values['nif'] = partner._split_vat(partner.vat, 'ES')[1]
    ...
    def _get_tbai_id(self):
        ...
        tbai_id_no_crc = '-'.join([
            'TBAI',
            str(self.env['res.partner']._split_vat(company.vat, 'ES')[1]),
