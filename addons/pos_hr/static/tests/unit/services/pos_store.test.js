...
test("checkPreviousLoggedCashier restores the employee from session storage", async () => {
    const store = await setupPosEnv();
...
test("basic cashier can edit prices when price control is unrestricted", async () => {
    const store = await setupPosEnv();
