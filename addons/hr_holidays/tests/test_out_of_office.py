...
class TestOutOfOffice(TestHrHolidaysCommon):
    ...
    def test_leave_ooo(self):
        ...
        employee_info = next(e for e in data["hr.employee"] if e["id"] == partner_info["employee_ids"][0])
        employee2_info = next(e for e in data["hr.employee"] if e["id"] == partner2_info["employee_ids"][0])
