...
class Certificate(models.Model):
    ...
    def _compute_issuer_cert_id(self):
        ...
        cert_data = {
            certificate: {
                'expected_ski': self._get_authority_key_identifier(loaded_certificate),
                'issuer_cn': issuer_cn,
            }
        ...
        if cert_data:
            ...
            for cert, data in cert_data.items():
                valid_parents = possible_parents.filtered(lambda x:
                    (not x.company_id or x.company_id == cert.company_id) and (x.subject_common_name == data['issuer_cn'])
                )

                if data['expected_ski']:
                    # Must match SKI exactly if expected_ski is present
                    matched_parents = valid_parents.filtered(
                        lambda parent: data['expected_ski'] == self._get_subject_key_identifier(load_certificate(parent))
                    )
                    if matched_parents:
                        cert.issuer_cert_id = matched_parents[:1].id
                elif valid_parents:
                    # Fallback to the first candidate if no expected_ski was explicitly set on the certificate
                    cert.issuer_cert_id = valid_parents[:1].id
    ...
    def _parse_chain_missing_ca_vals(self, vals):
        ...
        existing_keys = {(r['serial_number'], r['subject_common_name']) for r in existing_records}
        ...
        for ca_data in ca_data_list:
            ...
            if key not in existing_keys:
                ...
                # Add to existing_keys to handle duplicates within the same chain
