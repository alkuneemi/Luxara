...
class TestPdpCronSend(PdpTestCommon):
    def test_in_flow_sent_on_window_start_only(self):
        """Ready initial flow stays ready before window and auto-sends on window day."""
    ...
    def test_correction_created_on_last_day_with_errors(self):
        ...
        corrections = self.env['l10n.fr.pdp.reports.flow'].search([
        ...
        self.assertEqual(correction.transmission_type, 'rectificative')
        # v1.2: keep the excluded set on the initial flow (it drives invoice statuses)
        self.assertIn(invalid_invoice, flow.error_move_ids)
        # v1.2: rectificative is always a full replacement payload (never a delta)
        ...
        self.assertEqual(correction.state, 'pending', 'rectificative flow should wait until errors are fixed before it becomes ready')

    def test_in_waits_for_window_while_re_sends_immediately(self):
        """initial waits for window; subsequent rectificative sends immediately when ready."""
        inv = self._create_invoice(sent=True)
        flow_in = self._aggregate_company().filtered(lambda f: f.report_kind == 'transaction')[:1]
        self.assertEqual(flow_in.transmission_type, 'initial')
        ...
        self.assertEqual(flow_in.state, 'ready', 'initial should wait for its window')
        self.assertFalse(flow_in.transport_identifier)

        # Send initial then add a new invoice to produce CO ready to send immediately
        ...
        flow_re = self._aggregate_company().filtered(
            lambda f: f.report_kind == 'transaction' and f.id != flow_in.id and f.transmission_type == 'rectificative',
        )[:1]
        self.assertTrue(flow_re, 'rectificative flow should be created for new moves in the period')
        ...
        self.assertIn(flow_re.state, {'ready', 'completed', 'sent'}, 'rectificative should send immediately')
    ...
    def test_cron_generate_skips_non_pdp_company(self):
        ...
        other_company = self.env['res.company'].create({
            ...
            'l10n_fr_pdp_reports_enabled': False,
        })
        ...
        inv.action_post()
        self.env['l10n.fr.pdp.reports.flow.aggregator']._cron_generate_daily_flows()
        flows_other = self.env['l10n.fr.pdp.reports.flow'].search([('company_id', '=', other_company.id)])
    ...
    def test_bimonthly_window_sends_on_twenty_fifth(self):
        """Bimonthly initial flows send on window start when ready."""
