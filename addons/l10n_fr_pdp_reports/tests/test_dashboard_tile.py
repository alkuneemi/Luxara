...
class TestPdpDashboardTile(PdpTestCommon):
    def test_dashboard_tile_visibility_flags(self):
        ...
        self.company.l10n_fr_pdp_reports_enabled = False
    ...
    def test_dashboard_next_deadlines_from_flows(self):
        ...
        transaction_flow = self.env['l10n.fr.pdp.reports.flow'].create({
            'company_id': self.company.id,
        ...
        data = journal._get_journal_dashboard_data_batched()[journal.id]
        expected_tx = format_date(self.env, transaction_flow.next_deadline_end)
        expected_pay = format_date(self.env, payment_flow.next_deadline_end)
        self.assertEqual(data.get('pdp_transaction_due'), expected_tx)
        self.assertEqual(data.get('pdp_payment_due'), expected_pay)
