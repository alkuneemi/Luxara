...
class TestActivitySystrayBusNotify(TestActivityCommon):
    ...
    def test_notify_create_unlink_activities(self):
        ...
        for (
            ...
            with self.assertBus([], []):
                activities_2 = self.env['mail.activity'].create(user_activity_vals)
            with self.assertBus([], []):
                activities_2.unlink()
    ...
    def test_notify_update_activities(self):
        write_vals_all = [
            ...
            # removed from counter for current employee
            {'user_id': self.user_employee_2.id, 'date_deadline': datetime(2024, 1, 2, 15, 0, 0)},
            # removed then added back
            [{'date_deadline': datetime(2024, 1, 2, 15, 0, 0)}, {'date_deadline': datetime(2023, 12, 31, 15, 0, 0)}],
            [{'active': False}, {'active': True}],
            {},  # no "to be done" count change -> no notif
        ]

        expected_notifs = [
            # transfer 4 activities to the second employee :
            # employee 1 => no more activities -> -1 notif
            # employee 2 => 4 new activites -> +1 notif
            [
                ([(self.env.cr.dbname, user.partner_id._name, user.partner_id.id)], [{
                    "type": "mail.activity/updated",
                    "payload": {
            ...
            # transfer 4 activities to the second employee, with everything in the future
            # employee 1 => no more activities -> -1 notif
            # employee 2 => 4 new activites but all in the future -> no change
            [
                ...
                ([(self.env.cr.dbname, self.user_employee.partner_id._name, self.user_employee.partner_id.id) for _ in range(2)], [{
                    "type": "mail.activity/updated",
                    "payload": {
                        "count_diff": count_diff,
                    } | ({"activity_created": True} if count_diff > 0 else {"activity_deleted": True}),
                } for count_diff in (-1, 1)])
            ] for _ in range(2)
        ] + [
            [([], [])],  # no change -> no notif
        ]
