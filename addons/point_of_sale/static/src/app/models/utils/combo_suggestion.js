 ...
 * 2. `getComboChoiceLines()` and `getAllComboChoices()` format those computed combinations for the
 *    combo-selection popups.
 */
...
export class ComboSuggestion {
    ...
    /**
     * Splits applicable combos into direct matches and upsell suggestions, then sorts them for UI
     * consumption.
     */
    getPotentialCombos(order) {
        const potentialCombos = { applicable: [], upsell: [] };
     ...
     */
    getComboChoiceLines(models, combinations) {
        const comboItems = {};
    ...
    getAllComboChoices(potentialCombos) {
        const applicableCombos = potentialCombos.applicable.map((combo) => ({
            ...combo,
            lines: this.getComboChoiceLines(this.models, combo.combinations),
        }));
