...
class PosSession(models.Model):
    ...
    def _post_statement_difference(self, amount):
        if amount:
            ...
            default_taxes = counterpart_account.tax_ids
            if default_taxes:
                excluded_taxes = default_taxes.filtered(lambda t: not t.price_include)
                if excluded_taxes:
                    raise UserError(_(
                        "The taxes set on %(account)s cannot be applied to a cash difference: "
                        "the counted cash is always a gross amount, so the tax must be extracted from it. "
                        "Please configure the following tax(es) as 'Included in Price', or remove them from the account: %(taxes)s.",
                        account=counterpart_account.display_name,
                        taxes=", ".join(excluded_taxes.mapped('name')),
                    ))
                st_line_vals['line_ids'] = self._prepare_cash_diff_line_ids(
                    amount, counterpart_account, default_taxes,
    ...
    def _prepare_cash_diff_line_ids(self, amount, counterpart_account, taxes, name, date):
        ...
        taxes_res = taxes.compute_all(abs(amount), currency=journal_currency, quantity=1.0)
        sign = 1.0 if amount > 0 else -1.0
        cash_amount = sign * abs(amount)
