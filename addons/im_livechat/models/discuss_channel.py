...
class DiscussChannel(models.Model):
    ...
    def _add_members(self, **kwargs):
        ...
        if not skip_chatbot_current_step_reset:
            if agents := all_new_members.filtered(
                lambda m: m.livechat_member_type == "agent",
            ):
                store = Store.Stores()
                for channel in agents.channel_id:
                    agent_store = store[channel]
                    channel._notify_current_step_is_last(agent_store)
    ...
    def _notify_current_step_is_last(self, store):
        ...
        if self.sudo().chatbot_current_step_id:
            step_message = next((
                # sudo - chatbot.message.id: visitor can access chat bot messages.
                m.mail_message_id for m in self.sudo().chatbot_message_ids.sorted("id")
