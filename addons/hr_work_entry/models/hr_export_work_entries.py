...
class HrExportWorkEntries(models.TransientModel):
    ...
    has_one_company = fields.Boolean(compute="_compute_has_one_company")
    ...
    def _generate_export_file(self, company_id=None):
        ...
        company = self.env.company if not company_id else self.env['res.company'].search([('id', 'in', company_id)])
    ...
    def _generate_export_filename(self):
        time = datetime.now()
        formatted_time = ('_').join([str(time.day), str(time.month), str(time.year)])
        return 'workentries_' + formatted_time

    def _get_columns(self, company):
        columns = [
            ('Date', lambda args: args['date'].strftime('%d/%m/%Y')),
            ('Company External Code', lambda args: args['company'].external_code or ''),
            ('WET Internal Code', lambda args: args['we_collection']['work_entries'][0]['work_entry_type_id']['code'] or ''),
            ('WET Name', lambda args: args['we_collection']['work_entries'][0]['work_entry_type_id']['name'] or ''),
            ('WET External Code', lambda args: args['we_collection']['work_entries'][0]['work_entry_type_id']['external_code'] or ''),
            ('Employee External Code', lambda args: args['employee_id'].external_code or ''),
            ('Employee Name', lambda args: args['employee_id'].name),
            ('Description', lambda args: args['we_collection']['work_entries'][0]['work_entry_type_id']['description'] or ''),
            ('Duration', lambda args: str(int(args['we_collection']['duration'] // 60))),
        ]
        if not self.has_one_company:
            columns.append(('Company', lambda args: args['company'].name))
        return columns
    ...
    def _generate_line(self, employee_id, date, company, we_collection=None):
        ...
        work_entry.start_date;company.external_code;work_entry.code;work_entry.name;work_entry.external_code;employee.external_code;employee.name;description;work_entry.duration{;company}
        DATE(DD/MM/YYYY);INT;CHAR;CHAR;INT;CHAR;CHAR;INT(MINUTES){;CHAR}

        eg: 01/05/2026;3434;WORK100;Attendance;1212;5656;Mitchell Admin;desc;0800{;YourCompany}
        """
        columns = self._get_columns(company)
        args = {
            'date': date,
    ...
    def _get_contracts_by_employee(self, employee_ids=None, period_start=None, period_stop=None):
        ...
        contracts_by_employee = dict(self.env['hr.version']._read_group(
            domain=[
                ...
                ('contract_date_start', '<=', period_stop),
                '|',
    ...
    def _compute_has_one_company(self):
        allowed_companies = self.env['res.company'].search([('id', 'in', self.env.companies.ids)])
        for export in self:
            export.has_one_company = len(allowed_companies) == 1
