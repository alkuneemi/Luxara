...
class PayOnDelivery(models.TransientModel):
    ...
    @api.depends_context("payment_confirmed_order_ids")
    ...
    def _get_final_action(self):
        self.ensure_one()
        if self.order_ids._confirm_payment_on_delivery():
            return {
    ...
    def _get_confirmed_orders(self):
        return self.env["sale.order"].browse(
            self.env.context.get("payment_confirmed_order_ids", [])
        )

    @api.model
    def _with_confirmed_orders(self, orders):
        return self.with_context(
            payment_confirmed_order_ids=self._get_confirmed_orders().ids + orders.ids
        )
