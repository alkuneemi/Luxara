...
class GoogleCalendarService:
    ...
    def _get_or_create_calendar(self, calendar_data):
        ...
        if primary:
            primary_calendar = self.google_service.env.user.calendar_ids.filtered(lambda c: c.is_primary)
            if primary_calendar:
        ...
        new_calendar = self.google_service.env['calendar.calendar'].create({
            'google_id': None if primary else calendar_id,
            'name': 'Primary Calendar' if primary else calendar_data.get('summary'),
            'is_primary': primary,
            'is_readonly': calendar_data.get('accessRole') != 'owner',
            'user_id': self.google_service.env.user.id,
            'color': calendar_data.get('color', 1),
        })
        # Add newly synced calendar to the filters
        self.google_service.env['calendar.calendar.filter'].create(new_calendar._filter_values())
        return new_calendar
    ...
    def patch_calendar(self, calendar, values, token, timeout=TIMEOUT):
        url = "/calendar/v3/calendars/%s" % self.get_calendar_path(calendar)
        headers = {'Content-type': 'application/json', 'Authorization': 'Bearer %s' % token}
    ...
    def get_events(self, sync_token=None, token=None, event_id=None, calendar=None, search_params=None, timeout=TIMEOUT):
        url = f"/calendar/v3/calendars/{self.get_calendar_path(calendar)}/events"
        headers = {'Content-type': 'application/json'}
        params = {'access_token': token}
        if search_params:
    ...
    @requires_auth_token
    def insert(self, values, token=None, calendar=None, timeout=TIMEOUT, need_video_call=True):
        ...
        url = ("/calendar/v3/calendars/%s/events?conferenceDataVersion=%d&sendUpdates=%s" %
               (self.get_calendar_path(calendar), 1 if need_video_call else 0, "all" if send_updates else "none"))
        headers = {'Content-type': 'application/json', 'Authorization': 'Bearer %s' % token}
    ...
    @requires_auth_token
    def patch(self, event_id, values, token=None, calendar=None, timeout=TIMEOUT):
        ...
        url = ("/calendar/v3/calendars/%s/events/%s?sendUpdates=%s&conferenceDataVersion=1" %
               (self.get_calendar_path(calendar), event_id, "all" if send_updates else "none"))
        headers = {'Content-type': 'application/json', 'Authorization': 'Bearer %s' % token}
        self.google_service._do_request(url, json.dumps(values), headers, method='PATCH', timeout=timeout)

    @requires_auth_token
    def move(self, event_id, source_id, destination, send_updates, token=None, timeout=TIMEOUT):
        url = ("/calendar/v3/calendars/%s/events/%s/move?sendUpdates=%s&destination=%s" %
               (self.get_calendar_path(source_id), event_id, "all" if send_updates else "none", self.get_calendar_path(destination)))
        headers = {'Content-type': 'application/json', 'Authorization': 'Bearer %s' % token}
        return self.google_service._do_request(url, headers=headers, method='POST', timeout=timeout)

    @requires_auth_token
    def delete(self, event_id, token=None, calendar=None, timeout=TIMEOUT):
        url = "/calendar/v3/calendars/%s/events/%s?sendUpdates=all" % (self.get_calendar_path(calendar), event_id)
        headers = {'Content-type': 'application/json'}
        params = {'access_token': token}
    ...
    def get_calendar_path(self, calendar):
        if isinstance(calendar, str):
            return quote(calendar, safe='')
        return 'primary' if not calendar or calendar.is_primary else quote(calendar.google_id, safe='')
