...
class IrActionsReport(models.Model):
    ...
    def _render_qweb_pdf_prepare_streams(self, report_ref, data, res_ids=None):
        ...
        for (employee, payment_mode), group_expenses in expense_groups.items():
            ...
            bodies, _, header, footer, spec = self._prepare_html(html)
            summary_pdf_content = self._run_wkhtmltopdf(
                bodies,
                header=header,
                footer=footer,
                landscape=spec.get('landscape'),
                specific_paperformat_args=spec,
                set_viewport_size=spec.get('set_viewport_size'),
            )
                ...
                for attachment in self._prepare_local_attachments(attachments):
                    try:
                        if attachment.mimetype == 'application/pdf':
                            ...
                            img = Image.open(io.BytesIO(attachment.raw)).convert('RGB')
                            ...
                            title_text = expense.name or "Expense Receipt"
                            ...
                            output_pdf.append_pages_from_reader(OdooPdfFileReader(attachment_stream, strict=False))
                            streams_to_close.append(attachment_stream)
                        else:
                            continue
