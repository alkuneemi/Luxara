...
describe("restaurant pos_store.js", () => {
    ...
    test("restoreOrdersToOriginalTable restores course-linked lines", async () => {
        const store = await setupPosEnv();
        const sourceTable = store.models["restaurant.table"].get(1);
        const destinationTable = store.models["restaurant.table"].get(2);
        const sourceOrder = store.addNewOrder({ table_id: sourceTable });
        const course = store.addCourse();
        const product = store.models["product.template"].get(5);
        await store.addLineToOrder({ product_tmpl_id: product, qty: 2 }, sourceOrder);
        const line = sourceOrder.lines[0];
        ...
        sourceOrder.uiState.unmergeCourses = {
            [course.uuid]: {
                table_id: destinationTable.id,
                lines: [line.uuid],
                index: course.index,
        ...
        const restoredOrder = await store.restoreOrdersToOriginalTable(
            sourceOrder,
            destinationTable
        );

        expect(restoredOrder).not.toBe(false);
        expect(restoredOrder.table_id.id).toBe(destinationTable.id);
        expect(restoredOrder.lines).toHaveLength(1);
        expect(restoredOrder.lines[0].course_id).not.toBe(false);
    });
    ...
    describe("class DevicesSynchronisation", () => {
        ...
        test("paid state from server is synchronized", async () => {
            ...
            await store.deviceSync.readDataFromServer();
            const latestSyncedOrder = store.models["pos.order"].getAll()[0];
    ...
    test("order has no pending line changes after sending", async () => {
        ...
        const changes = store.getOrderChanges(order);
    ...
    test("handleSelectNamePreset sets floating order name from partner", async () => {
        const store = await setupPosEnv();
        const order = store.addNewOrder();
    ...
    test("handleSelectNamePreset prompts order name when partner is missing", async () => {
        const store = await setupPosEnv();
        const order = store.addNewOrder();
        const preset = store.models["pos.preset"].get(3);
        order.preset_id = preset;
        ...
        store.editFloatingOrderName = async (targetOrder) => {
            targetOrder.floating_order_name = "Walk-in";
        };

        await store.handleSelectNamePreset(order);

        expect(order.floating_order_name).toBe("Walk-in");
    });

    test("selecting customer fills floating order name without popup", async () => {
        const store = await setupPosEnv();
        const order = store.addNewOrder();
        ...
        store.editFloatingOrderName = async () => {
            popupOpened = true;
        };
    ...
    test("removing selected table order redirects to floor screen", async () => {
        const store = await setupPosEnv();
        const table = store.models["restaurant.table"].get(2);
        const order = store.addNewOrder({ table_id: table });
        store.setOrder(order);
    ...
    test("each restaurant floor keeps its own table set", async () => {
        ...
        expect(mainFloor.table_ids.map((table) => table.table_number)).toEqual([1, 2, 4]);
