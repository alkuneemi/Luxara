...
test("updateSOLines loads serial numbers when lots are enabled", async () => {
    const store = await setupPosEnv();
...
test("updateSOLines splits non-groupable lot lines and preserves converted price", async () => {
    const store = await setupPosEnv();
    patchWithCleanup(PosStore.prototype, {
        initLoadState() {
            return {
    ...
    expect(lines.map((line) => line.qty)).toEqual([2, 1]);
    expect(lines.map((line) => line.price_unit)).toEqual([12, 12]);
    expect(lines.map((line) => line.discount)).toEqual([15, 15]);
    expect(lines.map((line) => line.pack_lot_ids[0]?.lot_name)).toEqual(["LOT-1", "LOT-2"]);
});

test("updateSOLines keeps custom converted price for lot-tracked lines", async () => {
    const store = await setupPosEnv();
    patchWithCleanup(PosStore.prototype, {
        initLoadState() {
            return {
...
test("settling a sale order keeps ship later inactive by default", async () => {
    const store = await setupPosEnv();
...
test("settling lot-tracked lines loads SN/Lots", async () => {
    const store = await setupPosEnv();
    store.addNewOrder();
