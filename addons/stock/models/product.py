...
class ProductTemplate(models.Model):
    ...
    tracking = fields.Selection([
        ...
        string="Tracking", default='untracked', compute='_compute_tracking', inverse='_set_tracking',
    ...
    def _compute_tracking(self):
        for template in self:
            if not template.is_storable:
                template.tracking = 'untracked'
            elif template.tracking == 'untracked':
                template.tracking = 'none'

    def _set_tracking(self):
        for template in self:
            template.write({'is_storable': template.tracking != 'untracked'})
    ...
    def create(self, vals_list):
        ...
        for template, vals, qty in zip(product_templates, vals_list, product_tmpl_quantities):
            if template.tracking != 'untracked':
                template.is_storable = True
            elif vals.get('is_storable'):
                template.tracking = 'none'
            if qty > 0 and template.tracking == 'none':
                template.product_variant_id.qty_available = qty
