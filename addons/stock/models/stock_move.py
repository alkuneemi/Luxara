            ...
            for lot in move.lot_ids:
                if lot not in ls:
                    if move_line := mls_without_lots[:1]:  # Updates an existing line without serial/lot number.
                        ...
                        qty_to_reserve = (
                            1.0 if is_serial_tracked or qty_to_assign <= 0 or qty_assigned >= demand
    ...
    def _onchange_lot_ids(self):
        if self.has_tracking == 'serial':
            ...
            if newly_added_lots:
                ...
                # in case we have mls/quants without lots, it won't be reserved immediately and should
                # only be assigned to the first lot that took it
                empty_ml_taken, empty_quant_taken = False, False
                for lot in newly_added_lots:
                    available_qty = available_qty_by_lot.get(lot._origin.id, 0)
                    remaining_move_qty = demand - quantity
                    if mls_without_lots and not empty_ml_taken:
                        qty_to_add = 0
                        empty_ml_taken = True
                    elif remaining_move_qty > 0 and available_qty > 0:
                        qty_to_add = min(available_qty, remaining_move_qty)
                    elif remaining_move_qty > 0 and no_lot_available_qty > 0 and not empty_quant_taken:
                        qty_to_add = min(no_lot_available_qty, remaining_move_qty)
                        empty_quant_taken = True
                    else:
                        qty_to_add = 1
                    quantity += qty_to_add
