...
class TestHrEmployee(TestHrCommon):
    ...
    def test_set_user_on_new_employee(self):
        ...
        with mute_logger('odoo.sql_db'), self.assertRaises(ValueError), self.assertRaises(UniqueViolation), self.assertRaises(ValidationError):
    ...
    def test_user_creation_from_employee_multi_emails(self):
        ...
        employees |= self.env['hr.employee'].create({
            ...
            'work_email': f'"John Doe" <john@doe.com>',
        })
        employees |= self.env['hr.employee'].create({
