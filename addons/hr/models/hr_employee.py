...
class HrEmployee(models.Model):
    ...
    def _get_max_workable_hours(self, date_from, date_to, version) -> float:
        """
        Returns the max. number of hours the employee would have prestated if he
        worked during the given period under the given `version`.
        """
        tz = ZoneInfo(version.tz)

        # make sure date params are datetime in the employee's timezone
        ...
        # get all the possible attendances the employee could have prestated
        # (even if the contract started after the beggining of year)
        resources_per_tz = version._get_resources_per_tz()
        intervals = version.resource_calendar_id._attendance_intervals_batch(date_from, date_to, resources_per_tz)

        # sum all the attendances of the year as hours
        max_work_hours_per_year = 0
        for time_start, time_end, _ in intervals[self.resource_id.id]._items:
            time_worked = time_end - time_start
            max_work_hours_per_year += time_worked.total_seconds() / 3600

        return max_work_hours_per_year
    ...
    def _get_last_consecutive_versions(self, date_limit=date.max):
        ...
        def has_work_hours_between_versions(version_from, version_to):
            ...
            return bool(self._get_max_workable_hours(date_from, date_to, version_from))
    ...
    def _get_first_version_date(self, date_limit=None):
        if date_limit is None:  # support legacy context variable
            date_limit = self.env.context.get('before_date', date.max)
        versions = self._get_last_consecutive_versions(date_limit)
        return min(versions.mapped('date_start')) if versions else False

    def _get_first_contract_date(self, date_limit=None):
        if date_limit is None:  # support legacy context variable
            date_limit = self.env.context.get('before_date', date.max)
    ...
    def create(self, vals_list):
        ...
        for employee in employees:
            ...
            onboarding_notes_bodies[employee.id] = Markup(
                self.env._('<b>Congratulations!</b> May I recommend you to setup an <a href="%s">onboarding plan?</a>')
            ) % url
