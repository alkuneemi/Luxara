...
class HrWorkLocation(models.Model):
    ...
    def _unlink_except_used_by_employee(self):
        ...
        if self.env['hr.employee'].search_count(Domain.OR([('work_location_id', 'in', self.ids)] + [(day_field, 'in', self.ids) for day_field in DAYS]), limit=1):
            raise UserError(
                self.env._("You cannot delete work locations that are currently assigned to employees. Please reassign employees to different locations first."),
            )
