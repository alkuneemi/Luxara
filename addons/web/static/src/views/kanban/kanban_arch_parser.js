...
export class KanbanArchParser extends CardArchParser {
    parse(xmlDoc, models, modelName) {
        const cardArchInfo = super.parse(xmlDoc, models, modelName);

        const action = xmlDoc.getAttribute("action");
        const type = xmlDoc.getAttribute("type");
        cardArchInfo.openAction = action && type ? { action, type } : null;
        cardArchInfo.canOpenRecords = exprToBoolean(xmlDoc.getAttribute("can_open"), true);
        cardArchInfo.cardColorField = xmlDoc.getAttribute("highlight_color");
