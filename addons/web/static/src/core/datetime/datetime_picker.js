...
export class DateTimePicker extends Component {
    ...
    _adjustFocusedDateForPrecision() {
        const precisionDate = this.state.focusedDatesByPrecision.get(this.state.precision) || this.state.focusDate;

        if (this.state.precision === "days") {
            if (!isInRange(precisionDate, [this.state.focusDate, this.state.focusDate.endOf("month")])) {
                this.setFocusedDateByPrecision(this.state.precision, this.state.focusDate);
            } else {
                this.setFocusedDateByPrecision(this.state.precision, precisionDate);
            }
            return;
        }
        ...
        this.setFocusedDateByPrecision(
            this.state.precision,
            items.find((item) => isInRange(precisionDate, item.range))?.range[0] ?? this.state.focusDate
        );
    }
    ...
    _setupNavigation() {
        const focusDateCell = (navigator) => {
            const date = this.state.focusedDatesByPrecision.get(this.state.precision)?.toISODate();
        ...
        const onArrowKeyCallback = (navigator, key) => {
            ...
            switch (key) {
                case "arrowright":
                    offset = { [this.state.precision]: 1 };
                    break;
                case "arrowleft":
                    offset = { [this.state.precision]: -1 };
                    break;
                case "arrowup":
                    offset = { [this.state.precision]: -delta };
                    break;
                case "arrowdown":
                    offset = { [this.state.precision]: delta };
                    break;
            ...
            if (this.state.precision === "decades") {
                offset = { year: offset[this.state.precision] * 10 };
            }

            activeDate = activeDate.plus(offset);
