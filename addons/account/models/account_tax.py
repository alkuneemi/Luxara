...
class AccountTax(models.Model):
    ...
    def _add_and_round_raw_gross_total_excluded_and_discount(
        ...
        for base_line in base_lines:
            ...
            # Add the global discount back to retrieve the amount with only base_line['discount'] applied,
            # so dividing by 'discount_factor' yields the gross amount before any discount.
            raw_total_excluded_line_discount_only = raw_total_excluded
            if account_discount_base_lines:
                raw_total_excluded_line_discount_only -= sum(
            ...
            if discount_factor:
                raw_gross_total_excluded = raw_total_excluded_line_discount_only / discount_factor
