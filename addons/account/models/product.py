...
class ProductProduct(models.Model):
    ...
    def _get_line_price_unit(self, line, document_type, price=None):
        ...
        line_uom = line.uom_id if 'uom_id' in line._fields else line.product_uom_id  # add selected seller id uom if there is one
        ...
        if 'move_id' in line._fields:
            ...
            line_date = line.order_id.date_order
        ...
        apply_document_tax_mode = (not line.price_unit_json and (not price or get_opposite_tax_mode_value)) or (line.price_unit_json and line.price_unit_json['document_tax_mode'] != line.document_tax_mode)
        other_fiscal_position_set = (line.order_id.fiscal_position_id and line.order_id.fiscal_position_id != line.company_id.domestic_fiscal_position_id)
        new_product_set = product and (not line.price_unit_json or not line.price_unit_json['product_id'] or line.price_unit_json['product_id'] != line.product_id.id)

        # We need to make sure the quantity hasn't changed if we want to use the price on the line
        if line.price_unit_json and line.product_qty == line.price_unit_json['qty']:
            price = line.price_unit if line.price_unit else price

        if not price and new_product_set:
            product_values = None
        ...
        # Handling cases where a new line is set in a document with a fiscal position can be affected by an including tax
        if other_fiscal_position_set and product_values and new_product_set:
            product_values['taxes'] = line.product_id.supplier_taxes_id
        ...
        return product._get_tax_included_unit_price(
            ...
            fiscal_position=line.order_id.fiscal_position_id if other_fiscal_position_set else None,
            product_uom=line_uom if apply_uom else None,
            document_tax_mode=line.document_tax_mode if apply_document_tax_mode else None,
            get_opposite_tax_mode_value=get_opposite_tax_mode_value or document_tax_mode_changed,
