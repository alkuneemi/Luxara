...
class AccountMove(models.Model):
    ...
    def _check_review_state_access(self, review_state):
        """Raise a ValidationError if the current user cannot modify a record with the given review_state.
        ...
        if review_state == 'supervised' and not is_user_able_to_supervise:
            raise ValidationError(_("This entry has already been supervised. You need the accounting manager role to change it."))
        if (review_state == 'reviewed' or not review_state) and not is_user_able_to_review:
    ...
    def write(self, vals):
        ...
        is_user_able_to_review, _ = self._get_review_state_access_groups()
