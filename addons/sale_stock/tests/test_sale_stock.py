...
class TestSaleStock(TestSaleStockCommon, ValuationReconciliationTestCommon):
    ...
    def test_sale_order_cancel_with_partial_backorder_flow(self):
        """
        1. Create an SO with quantity 5 and confirm it.
        2. Validate the pick for 2 units and generate a backorder for the remaining 3.
        3. Validate the pack for 2 units.
        4. Validate the backorder pick for 3 units.
        5. Cancel the SO.
        6. Verify the state of the reverse Pack and Pick, and the stock after validation.
        """
        warehouse = self.company_data['default_warehouse']
        warehouse.delivery_steps = 'pick_pack_ship'
        stock_location = warehouse.lot_stock_id
        ...
        sale_order.action_cancel()
        reverse_pack = sale_order.picking_ids.filtered(lambda p: p.state == 'assigned' and p.picking_type_id.id == warehouse.pack_type_id.id)
        reverse_pick = sale_order.picking_ids.filtered(lambda p: p.state == 'assigned' and p.picking_type_id.id == warehouse.pick_type_id.id)
