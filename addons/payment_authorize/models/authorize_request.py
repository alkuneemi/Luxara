...
class AuthorizeAPI:
    ...
    def _format_response(self, response, operation):
        if response and response.get("err_code"):
            return {"x_response_code": self.AUTH_ERROR_STATUS, "x_response_reason_text": response.get("err_msg")}
        else:
            return {
        ...
        if "profile" not in tx_data:
            ...
            bill_to = {
                "billTo": {
                    ...
                    "country": tx.partner_country_id.name[:60] or "",
                }
            }
    ...
    def void(self, transaction_id):
        ...
        response = self._make_request(
            "createTransactionRequest",
            {"transactionRequest": {"transactionType": "voidTransaction", "refTransId": transaction_id}},
        )
