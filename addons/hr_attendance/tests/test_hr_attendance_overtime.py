...
class TestHrAttendanceOvertime(HttpCase):
    ...
    def test_overtime_line_access_rules(self):
        ...
        all_lines = overtime_line.search([('attendance_id.employee_id', 'in', (own_reader_employee | officer_employee | self.other_employee).ids)])
        self.assertEqual(len(all_lines.attendance_id.employee_id), 3)

        own_reader_lines = overtime_line.with_user(own_reader_user).search([])
        self.assertEqual(own_reader_lines.attendance_id.employee_id, own_reader_employee)

        officer_lines = overtime_line.with_user(officer_user).search([])
        self.assertEqual(officer_lines.attendance_id.employee_id, officer_employee | self.other_employee)

        admin_lines = overtime_line.with_user(self.user).search([])
        self.assertIn(own_reader_employee, admin_lines.attendance_id.employee_id)
        self.assertIn(officer_employee, admin_lines.attendance_id.employee_id)
        self.assertIn(self.other_employee, admin_lines.attendance_id.employee_id)
