...
class MailCallArtifact(models.Model):
    """Call Artifact represents a discrete product of a call (audio, transcript etc.)
    ...
    def _check_artifacts_overlap(self, artifacts):
        """Checks if the provided artifacts overlap in time"""
        candidates = sorted(
    ...
    def _get_related_call(self):
        """Returns the parent call record (discuss.call.history, voip.call, etc.)"""
        self.ensure_one()
