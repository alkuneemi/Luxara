...
let collator;
try {
    collator = new Intl.Collator(user.lang ?? "en-US");
} catch {
...
partnerCompareRegistry.add(
    ...
    function name(p1, p2, { searchTerm, thread }) {
        ...
        // 1. prioritizes partners with a name over those without
        if (name1 && !name2) {
            return -1;
        }
        if (!name1 && name2) {
        ...
        return collator.compare(name1, name2) || undefined;
    },
    { sequence: 50 }
);
...
partnerCompareRegistry.add(
    ...
    function email(p1, p2, { searchTerm }) {
        ...
        // 1. prioritizes partners with an email over those without
        if (p1.email && !p2.email) {
            return -1;
        }
        if (!p1.email && p2.email) {
        ...
        // 3. locale-sensitive alphabetical order
        return collator.compare(p1.email, p2.email) || undefined;
    },
    { sequence: 55 }
);
