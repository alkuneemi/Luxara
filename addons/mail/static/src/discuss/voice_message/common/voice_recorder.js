 ...
 * @param {Function} [params.onRecordReady] Callback when recording is finished. Defaults to attachmentUploader.uploadFile.
 */
export function useVoiceRecorder(params = {}) {
    const maxDuration = params.maxDuration ?? 60;
    const component = useComponent();
    const onRecordReady = params.onRecordReady || ((file) => {
        return component.attachmentUploader.uploadFile(file, { voice: true });
    });
    /** @type {MediaStream} */
    let microphone;
    /** @type {number} */
    ...
    async function startRecording() {
        ...
        processor.port.onmessage = (e) => {
            ...
            if (elapsedSeconds >= maxDuration) {
                notification.add(
                    _t("The duration of voice messages is limited to %s minute(s).", Math.round(maxDuration / 60)),
                    {
                        type: "warning",
                    }
                );
