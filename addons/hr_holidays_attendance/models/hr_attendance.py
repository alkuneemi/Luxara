...
class HrAttendance(models.Model):
    ...
    def _gantt_compute_max_work_hours_within_interval(self, employee, start, stop):
        ...
        if not employee.is_flexible or employee.is_fully_flexible:
            return res
        ...
        attendance_intervals = employee.resource_calendar_id._work_intervals_batch(start, stop, employee.resource_id, compute_leaves=False)[employee.resource_id.id]
        leaves_intervals = employee.resource_calendar_id._leave_intervals_batch(start, stop, employee.resource_id, [('holiday_id.request_unit_hours', '=', True)])[employee.resource_id.id]
