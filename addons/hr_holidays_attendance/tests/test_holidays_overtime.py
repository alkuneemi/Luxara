...
class TestHolidaysOvertime(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.hourly_leave_type = cls.env['hr.leave.type'].create({
            'name': 'Hourly Time Off',
            'company_id': cls.company.id,
            'requires_allocation': 'no',
        })
        cls.worked_time_leave_type = cls.env['hr.leave.type'].create({
    ...
    def test_auto_check_out_ignores_worked_time_hourly_leave(self):
        ...
        leave = self.env['hr.leave'].create({
            'name': 'Training',
            'employee_id': self.employee.id,
            'holiday_status_id': self.worked_time_leave_type.id,
            'request_unit_hours': True,
            'request_date_from': datetime(2024, 12, 3),
            'request_date_to': datetime(2024, 12, 3),
