...
export class CrmSearchModel extends SearchModel {
     ...
     */
    async _updateSwitcherSelection(teamId) {
        this.state.switcherTeamId = teamId;
        browser.localStorage.setItem("crm.switcher_team_id", teamId);
    ...
    _getDomain(params = {}) {
        ...
        const switcherDomain = new Domain([["team_id", "in", teamIds]]);
        if (params.raw) {
            return Domain.and([domain, switcherDomain]);
        }
        return Domain.and([new Domain(domain), switcherDomain]).toList(this.domainEvalContext);
    }
}
