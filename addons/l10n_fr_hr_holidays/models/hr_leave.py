    ...
    def _get_fr_date_from_to(self, date_from, date_to):
        ...
        if self.work_entry_type_request_unit != 'hour':
            ...
            def adjust_date_range(date_from, date_to, from_period, to_period, attendance_ids, employee_id):
                period_ids_from = attendance_ids._filter_by_date(date_from.date()).filtered(lambda a: a.day_period in from_period)
                period_ids_to = attendance_ids._filter_by_date(date_to.date()).filtered(lambda a: a.day_period in to_period)
                if period_ids_from:
