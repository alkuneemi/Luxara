...
class AccountMove(models.Model):
    ...
    def button_update_pajakio_status(self):
    ...
    def _l10n_id_pajakio_update_status(self):
        ...
        data = response.get("data")
        for move in self.filtered(lambda inv: inv.l10n_id_pajakio_transaction_id):
            result = data.get(move.l10n_id_pajakio_transaction_id)
            status = result["status"]
            # if there's an error/misisng data, log it and continue to other transactions
            if status == "missing":
                move.message_post(body=_("The following Pajak.io transaction is not found in IAP service"))
                continue
            elif status == "error":
                err_message = result.get("error")
                move.message_post(body=_("Pajak.io update failed with the following repsonse: %s", err_message))
                continue
            detail = result["data"]
