...
test("getProductPriceInfo", async () => {
    ...
    // Fiscal position must change display price; preset still applies fp if order.fiscal_position_id is cleared.
    ...
    order.fiscal_position_id = false;
    expect(store.getProductDisplayPrice(product5)).toBe(displayAfterStripFp);
