...
const mockProcessOrderWithTampering = (authoritativePayload) => {
...
describe("frontend/backend price consistency", () => {
    test("combo prices between frontend and backend", async () => {
        const store = await setupSelfPosEnv();
    ...
    test("price between frontend and backend", async () => {
        const store = await setupSelfPosEnv();
    ...
    test("prices are immutable from frontend", async () => {
        const store = await setupSelfPosEnv();
        const product = store.models["product.template"].get(5);
        ...
        mockProcessOrderWithTampering(authoritativePayload);
    ...
    test("pricelist should not be changed from frontend", async () => {
        const store = await setupSelfPosEnv();
        const product = store.models["product.template"].get(5);
    ...
    test("pricelist price between frontend and backend", async () => {
        const store = await setupSelfPosEnv();
        const product = store.models["product.template"].get(5);
        const variant = product.product_variant_ids[0];
    ...
    test("fiscal position between frontend and backend", async () => {
        const store = await setupSelfPosEnv();
        const product = store.models["product.template"].get(5);
        const variant = product.product_variant_ids[0];
...
test("verifyCart handles unavailable combo and standalone products", async () => {
    const store = await setupSelfPosEnv();
    const comboTemplate = store.models["product.template"].get(7);
    const comboItem1 = store.models["product.combo.item"].get(1);
    const comboItem3 = store.models["product.combo.item"].get(3);
