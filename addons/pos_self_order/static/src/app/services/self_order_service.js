...
export class SelfOrder extends Reactive {
    ...
    getProductPriceInfo(productTemplate, product) {
        ...
        const pricelist =
            order.pricelist_id || order.preset_id?.pricelist_id || this.config.pricelist_id;
        const productForPricing =
            product || productTemplate.product_variant_ids?.[0] || productTemplate;
        const price = productTemplate.getPrice(pricelist, 1, 0, false, productForPricing);

        const fiscalPosition =
            order?.fiscal_position_id || order?.preset_id?.fiscal_position_id || false;
        // Taxes computation.
        const taxesData = productForPricing.getTaxDetails({
            overridedValues: {
                price,
                fiscalPosition,
            },
