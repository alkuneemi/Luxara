...
test("applied custom color should be shown in colorpicker after switching tab", async () => {
    ...
    await contains(hexInputEl).edit(newColor, { confirm: false });
...
test("gradient picker correctly shows the current selected gradient", async () => {
    ...
    const angleIframeEl = queryOne(".o_font_color_selector iframe.o_angle_iframe");
    const angleInputEl = await waitUntil(() => {
        const input = angleIframeEl.contentWindow.document?.querySelector(
...
describe("keyboard navigation", () => {
    test("update saturation and brightness picker with keys", async () => {
        ...
        const hexInputEl = await waitUntil(() => {
            const input =
                hexIframeEl.contentWindow.document?.querySelector("input[name='hex_input']");
            return input?.value && input;
        });
        expect(hexInputEl).toHaveValue("#FF0000");
        await press("ArrowUp");
