...
import { Youtube } from "@html_editor/main/media/video/youtube";
import { Dailymotion } from "@html_editor/main/media/video/dailymotion";
import { Vimeo } from "@html_editor/main/media/video/vimeo";
import { GDriveVideo } from "@html_editor/main/media/video/gdrive_video";
import { Instagram } from "@html_editor/main/media/video/instagram";
import { Facebook } from "@html_editor/main/media/video/facebook";
import { Twitch } from "@html_editor/main/media/video/twitch";
import { Loom } from "@html_editor/main/media/video/loom";
...
class VideoOption extends Component {
    ...
    static props = {
        ...
        onTimeValueChanged: Function,
        onTimeValueInputed: Function,
        value: { type: Boolean, optional: true },
        name: { type: String, optional: true },
        timeValue: { type: String, optional: true },
    };

    get isTimeInputVisible() {
        return this.props.value && this.props.timeValue !== undefined;
    }

    get timeValue() {
        return this.props.timeValue || "0:00";
    }
}
...
export class VideoSelector extends Component {
    ...
    setup() {
        ...
        this.OPTIONS = {
            ...
            hideFullscreen: {
                ...
                isVisible: () => !this.state.options?.hideControls?.value,
            },
            isVertical: {
                label: _t("Vertical"),
            },
        ...
        onMounted(async () => {
            ...
            if (media) {
                ...
                // Deprecated oeExpression store the url without 'http:' for some reason.
        ...
        this.refreshVideoData = debounce(this._refreshVideoData.bind(this), 25);
        // A longer debounce time is preferred after the time input
        // to avoid template rendering interfering with user inputting data
        this.refreshTimeInput = debounce(this.onTimeValueChanged.bind(this), 2000);
    ...
    get visibleVideoOptions() {
        ...
        for (const [name, config] of Object.entries(this.OPTIONS)) {
            const option = { id: name, ...config };
            const isVisible = config?.isVisible ? config?.isVisible() : true;
            option["value"] = !!this.state.options?.[name]?.value;
            if (name === "startFrom") {
                option["timeValue"] = this.convertSecondsToTimestamp(
                    this.state.options?.[name]?.timeValue
                );
            }
            if (isVisible && platformOptionsConfig?.[name]) {
                options.push(option);
            }
        }
        return options;
    ...
    onOptionToggled(optionId) {
        this.updateOption(optionId, !this.state.options?.[optionId]?.value);
    }

    updateOption(optionId, value) {
        this.props.selectMedia({}); // Temporarily invalidate the selected video until the url is parsed.
        const option = this.state.options?.[optionId] || {};
        this.state.options[optionId] = { ...option, value };
        this.refreshVideoData();
    }

    onTimeValueChanged(ev, optionId) {
        this.state.options[optionId].timeValue = this.convertTimestampToSeconds(ev.target.value);
        this._refreshVideoData();
    }

    onTimeValueInputed(ev, optionId) {
        this.props.selectMedia({}); // Temporarily invalidate the selected video the time is updated.
        this.refreshTimeInput(ev, optionId);
    }
    ...
    parseOptionsFromUrl(url) {
        ...
        for (const option of this.visibleVideoOptions) {
            if (videoData.options?.[option.id]) {
                ...
                if (option.timeValue) {
                    this.state.options[option.id].timeValue = videoData.options[option.id];
                }
            }
        }
        ...
        if (this.props.isForBgVideo) {
            ...
            for (const option of this.visibleVideoOptions) {
                if (option.value !== undefined) {
                    if (option.timeValue) {
                        forcedOptions[option.id] = this.convertTimestampToSeconds(option.timeValue);
                    } else {
                        forcedOptions[option.id] = option.value;
                    }
                }
            }
        }
    ...
    async updateVideoPreview(videoData) {
        ...
        options.isVertical = this.state.options?.isVertical?.value;
