 ...
 */
/**
 * @typedef { import("./history_plugin").HistoryCommit } HistoryCommit
 * @typedef { import("./history_plugin").HistoryCommitType } HistoryCommitType
 * @typedef { import("./history_plugin").HistoryCommitId } HistoryCommitId
 ...
 */
/**
 * @typedef { Exclude<NativeMutationType, "childList"> | "classList" | "add" | "remove" | "custom" } EditorMutationType
 */
/**
 * Editor Mutations
 * ----------------
 ...
 * @typedef { ((
 *    value: string,
 *    params: { mutation: SerializedMutation<"attributes">, ensureNewMutations: boolean, wasReversed: boolean }
 *  ) => string)[] } attribute_mutation_to_apply_value_processors
 ...
 * @typedef { ((value: string, mutation: NativeMutation<"attributes">) => void)[] } attribute_mutation_value_processors
 * @typedef { ((records: SerializedMutation[]) => void)[] } on_pending_mutations_staged_handlers
...
export class DomObserverPlugin extends Plugin {
    ...
    resources = {
        ...
        pending_commit_data_processors: withSequence(0, (data) => this.processPendingData(data)),
        ...
        mutations = mutations.filter((mutation) => {
            if (mutation.type === NATIVE_MUTATION_TYPES.CHILD_LIST) {
                ...
                if (
                    ...
                    if (oldId) {
                        this.dependencies.domReferenceMap.set(oldId, firstAdded);
                        return false;
                    }
                }
            }
    ...
    processAttributesMutation(mutation) {
        ...
        if (mutation.attributeName === "class") {
            ...
            const value = this.processThrough(
                "attribute_mutation_value_processors",
    ...
    /**
     * Return an object with the added and removed classes in a mutation on a class attribute.
     *
     * @param { NativeMutation<"attributes"> } mutation
