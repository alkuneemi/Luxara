 ...
 * @typedef {((commit: HistoryCommit, options?: { ensureNewMutations?: boolean, restoreSelection?: boolean }) => void)[]} on_apply_commit_handlers
 * @typedef {((commit: HistoryCommit, options?: { ensureNewMutations?: boolean, restoreFocus?: boolean }) => void)[]} on_revert_commit_handlers
 * @typedef {((revertedCommit: HistoryCommit<"standard" | "redo"> | undefined) => void)[]} on_history_commit_undone_handlers
 * @typedef {((revertedCommit: HistoryCommit<"undo"> | undefined) => void)[]} on_history_commit_redone_handlers
 ...
 *
 * @typedef {((commit: HistoryCommit) => boolean | undefined)[]} is_commit_reversible_predicates
 * @typedef {((commit: HistoryCommit) => boolean | undefined)[]} has_commit_changes_predicates
 *
 * @typedef { ((data: HistoryCommitData) => HistoryCommitData | undefined)[] } pending_commit_data_processors
 * @typedef {((data: HistoryCommitData<"standard">) => HistoryCommitData<"standard"> | undefined)[]} snapshot_commit_data_processors
 * @typedef { ((data: HistoryCommitData<"savePoint">) => HistoryCommitData<"savePoint"> | void)[] } save_point_data_processors
 */
...
export class HistoryPlugin extends Plugin {
     ...
     */
    undo() {
        this.revise(HISTORY_COMMIT_TYPES.UNDO);
    }
     ...
     */
    redo() {
        this.revise(HISTORY_COMMIT_TYPES.REDO);
    }
    ...
    appendCommit(commit) {
        ...
        // this.sanitize();
        if (this.checkPredicates("has_commit_changes_predicates", commit) ?? false) {
    ...
    revise(type) {
        ...
        let revertedCommit;
        for (revertedCommit of this.getNextRevisionCommits(type)) {
     ...
     */
    canUndo() {
        return this.getNextRevisionIndex(HISTORY_COMMIT_TYPES.UNDO) > 0;
    }
     ...
     */
    canRedo() {
        return this.getNextRevisionIndex(HISTORY_COMMIT_TYPES.REDO) > 0;
    }
    ...
    /**
     * Return true if a preview is ongoing, false otherwise.
     *
     * @returns { boolean }
     */
    getIsPreviewing() {
