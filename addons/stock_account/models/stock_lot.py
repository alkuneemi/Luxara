...
class StockLot(models.Model):
    ...
    def _compute_value(self):
        ...
        for cost_method, lots in self.env['stock.lot'].browse(cost_method_valuated_lot_ids).grouped(lambda lot: lot.product_id.cost_method).items():
            if cost_method == 'average':
                ...
                qty_available_by_lot_id = defaultdict(int)
                for lot in lots:
                    qty_available = lot.with_context(warehouse_id=False).product_qty
                    qty_available_by_lot_id[lot.id] = qty_available
                fifo_value_by_lot_id = self.env['product.product'].with_context(warehouse_id=False)._run_fifo_batch_for_lots(lots.with_context(warehouse_id=False), qty_available_by_lot_id, at_date=at_date)
                for lot in lots:
                    fifo_value = fifo_value_by_lot_id[lot.id]
                    qty_available = qty_available_by_lot_id[lot.id]
                    qty_valued = lot.product_qty
