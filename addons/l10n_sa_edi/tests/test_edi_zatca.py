...
class TestEdiZatca(TestSaEdiCommon):
    ...
    def test_invoice_global_rounding_payable_amount(self):
        ...
        edi_handler = self.env['account.edi.xml.ubl_21.zatca']
        prepaid_vals = edi_handler._l10n_sa_get_prepaid_amount(final, {})
        ...
        self.assertEqual(prepaid_vals['tax_amount'], expected_tax,
            f"Tax amount should be {expected_tax} (correct hundredth rounding), got {prepaid_vals['tax_amount']}")

        monetary_vals = edi_handler._l10n_sa_get_monetary_vals(final, {
