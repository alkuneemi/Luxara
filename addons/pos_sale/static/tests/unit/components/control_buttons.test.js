...
test("onClickQuotation filters sale orders by unpaid amount and POS currency", async () => {
...
test("onClickQuotation adds selected partner filters to the dialog", async () => {
    const store = await setupPosEnv();
...
test("onClickQuotation forwards the selected sale order id to the POS store", async () => {
    const store = await setupPosEnv();
    store.addNewOrder();
    const selectedIds = [];
