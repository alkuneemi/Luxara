...
class SaleOrder(models.Model):
    ...
    def _compute_amount_paid(self):
        ...
        for sale_order in self:
            pos_amount_paid = sum(sale_order.order_line.filtered(lambda l: not l.display_type).mapped('pos_order_line_ids.price_subtotal_incl'))
            sale_order.amount_paid += pos_amount_paid

    @api.depends('transaction_ids.state', 'transaction_ids.amount', 'order_line', 'amount_total', 'order_line.invoice_lines.parent_state', 'order_line.invoice_lines.price_total')
