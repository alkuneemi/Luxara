...
class AccountEdiCommon(models.AbstractModel):
    ...
    def _import_partner(self, company_id, name, phone, email, vat, *, additional_identifiers=None, postal_address={}, **kwargs):
        """ Retrieve the partner, if no matching partner is found, create it (only if he has a vat and a name) """
        logs = []
        partner = self.env['res.partner'] \
            .with_company(company_id) \
            ._retrieve_partner(name=name, phone=phone, email=email, vat=vat, additional_identifiers=additional_identifiers)
