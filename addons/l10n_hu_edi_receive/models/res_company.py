...
class ResCompany(models.Model):
    ...
    def l10n_hu_edi_show_nav_sync_button(self):
        return any(company.l10n_hu_edi_server_mode in ('test', 'production') for company in self.env.companies)
    ...
    def l10n_hu_edi_receive_inbound_invoices(self, datetime_from, datetime_to):
        ...
        with L10nHuEdiConnection(self.env) as connection:
            while fetch_next_page:
                digests, fetch_next_page = connection.query_invoice_digest(credentials, datetime_from, datetime_to, page)
                moves_vals_list, post_process_data_list = connection.query_invoice_data(credentials, digests)
