...
class MrpProduction(models.Model):
    ...
    show_produce = fields.Boolean(compute='_compute_show_produce', help='Technical field to check if the produce button can be shown')
    show_generate_bom = fields.Boolean('Show Generate BOM', compute='_compute_show_generate_bom')
    show_produce_all = fields.Boolean(compute='_compute_show_produce', help='Technical field to check if the close production button can be shown')
    ...
    @api.depends('picking_type_id', 'product_qty', 'qty_producing', 'state', 'uom_id')
    def _compute_show_produce(self):
        for production in self:
            state_ok = production.state in ('confirmed', 'progress', 'to_close')
            if uom := production.uom_id:
                partial_demand = not uom.is_zero(production.qty_producing) and uom.compare(production.qty_producing, production.product_qty) < 0
            else:
                partial_demand = 0 < production.qty_producing < production.product_qty
            # Show 'Produce' as the primary button and 'Close Production' as the secondary button
            production.show_produce = state_ok and partial_demand and production.picking_type_id.create_backorder != 'never'
            # Only show the 'Close Production' primary button
            production.show_produce_all = state_ok and not production.show_produce
    ...
    def _get_consumption_issues(self):
        ...
        for order in self:
            ...
            # Ignore `qty_producing` if not producing partially. (ie the user clicked 'Close Production')
            bomless_factor = 1 if self.env.context.get('close_production') else order.qty_producing / order.product_qty
            if order.bom_id:
                bom_factor = (order.product_qty if self.env.context.get('close_production') else order.qty_producing) \
                    / order.bom_id.uom_id._compute_quantity(order.bom_id.product_qty, order.uom_id, round=False)
