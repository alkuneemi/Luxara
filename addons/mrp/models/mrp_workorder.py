...
class MrpWorkorder(models.Model):
    ...
    def _compute_duration_expected(self):
        for workorder in self:
            ...
            # or if the UoM has been changed on the production:
    ...
    def _get_duration_expected(self, alternative_workcenter=False, ratio=1):
        ...
        product_uom = self.production_id.product_id.uom_id
        capacity, setup, cleanup = self.workcenter_id._get_capacity(self.product_id, product_uom, self.production_bom_id.product_qty or 1)
        ...
        qty_production = self.product_uom_id._compute_quantity(self.qty_producing or self.qty_production, product_uom)
        ...
        if alternative_workcenter:
            ...
            if duration_expected_working < 0:
                duration_expected_working = 0
            capacity, setup, cleanup = alternative_workcenter._get_capacity(self.product_id, product_uom, self.production_bom_id.product_qty or 1)
            cycle_number = float_round(qty_production / capacity, precision_digits=0, rounding_method='UP')
