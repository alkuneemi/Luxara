...
class TestPaymentTransaction(QFPayCommon):
    ...
    def test_prepare_payment_intent_payload_includes_mchid_when_set(self):
        """Test that the merchant id is included only when configured on the provider."""
        tx = self._create_transaction("direct")
        tx.provider_id.qfpay_mchntid = "mch-123"
        return_url = urljoin(tx.provider_id.get_base_url(), const.PAYMENT_RETURN_ROUTE)

        payload = tx._qfpay_prepare_payment_intent_payload("75000", return_url)

        self.assertEqual(payload["mchid"], "mch-123")

    def test_prepare_payment_intent_payload_omits_mchid_when_not_set(self):
        """Test that the merchant id is omitted when not configured on the provider."""
        tx = self._create_transaction("direct")
        return_url = urljoin(tx.provider_id.get_base_url(), const.PAYMENT_RETURN_ROUTE)

        payload = tx._qfpay_prepare_payment_intent_payload("75000", return_url)

        self.assertNotIn("mchid", payload)
