...
The provider requires an `App Code` and an `App Key`, and supports an optional `Merchant ID`.
...
Set the provider to test mode and configure the QFPay test credentials (`App Code`, `App Key`,
and, if applicable, `Merchant ID`).
