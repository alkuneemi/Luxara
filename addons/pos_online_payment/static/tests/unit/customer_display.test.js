...
import "@pos_online_payment/overrides/customer_display_overrides/customer_display_adapter";

definePosModels();

test("formatOrderData includes online payment data for customer display", async () => {
...
test("formatOrderData reflects online payment lifecycle for customer display", async () => {
    const store = await setupPosEnv();
    const adapter = new CustomerDisplayPosAdapter();
...
test("customer display UI renders online payment QR card", async () => {
    const imgUri =
