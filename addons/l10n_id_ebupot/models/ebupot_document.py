...
class EBupotDocument(models.Model):
    ...
    def action_download(self):
        ...
        return {
            ...
            'url': f'/l10n_id_ebupot/download_attachments/{",".join(map(str, self.attachment_ids.ids))}',
        }
    ...
    def _generate_ebupot_invoice(self):
        """ Generate E-Bupot XML grouped by vendor + payment month """
        payment_groups = self.payment_ids.prepare_ebupot_vals()
        xml_results = []
        for idx, group in enumerate(payment_groups, start=1):
            partner = self.env['res.partner'].browse(group['partner_id'])
            ...
            xml_content = self.env['ir.qweb']._render('l10n_id_ebupot.ebupot_template', {'data': all_vals, 'TIN': self.company_id.vat})
            ...
            xml_results.append({
                'tax': f"{partner.name or 'Unknown'}_{payment_month}_ebupot_{idx}",
                'xml': xml_bytes
            })
        return xml_results
