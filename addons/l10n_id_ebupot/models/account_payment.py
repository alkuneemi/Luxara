...
class AccountPayment(models.Model):
    ...
    l10n_id_ebupot_document_xml = fields.Many2one('l10n_id_ebupot.document', readonly=True, copy=False, string="E-Bupot Document (Coretax)")
    ...
    def download_ebupot(self):
        ...
        if not self.l10n_id_ebupot_document_xml:
            self.l10n_id_ebupot_document_xml = self.env['l10n_id_ebupot.document'].create({
        ...
        # They could get a file with additional invoices for example. In this case, we redirect them to the document view to make it clearer.
            ...
            action_error = {
                ...
                'view_mode': 'list',
                'res_model': 'l10n_id_ebupot.document',
    ...
    def prepare_ebupot_vals(self):
        ...
        for bill in bills:
            ...
            for payment in payments:
                ...
                group_key = (partner.id, payment_month)
                ...
                for line in bill.invoice_line_ids:
                    ...
                    tax = pph_taxes[0]
                    if tax.id not in grouped_data[group_key]:
                        grouped_data[group_key][tax.id] = {
                            'tax': tax,
                            'vals': []
                        }
                    ...
                    grouped_data[group_key][tax.id]['vals'].append(vals)

        if not grouped_data:
            raise UserError(_("No PPh data found to generate E-Bupot."))
        result = []
        for (partner_id, payment_month), group in grouped_data.items():
            result.append({
