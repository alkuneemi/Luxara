...
class WebsiteVisitor(models.Model):
    ...
    def visitor_view_action_button(self):
        ...
        """
        self.ensure_one()
        if not self.env.context.get('model_name'):
            raise UserError(_("Model name is required in context to open visitor track views."))
        return {
            'type': 'ir.actions.act_window',
            'name': self.env.context.get('title', _('Views History')),
            'res_model': 'website.track',
            'view_mode': 'list,graph',
            'views': [
                (self.env.ref("website.website_visitor_track_view_base_list").id, 'list'),
                (self.env.ref("website.website_visitor_track_view_base_graph").id, 'graph'),
            ],
            'domain': [
                ('visitor_id', '=', self.id),
                ('res_model', '=', self.env.context.get('model_name')),
            ],
        }
