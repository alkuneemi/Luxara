...
class TestWebsiteDefaultWebsiteConsistency(TransactionCase):
    ...
    def test_02_ensure_default_on_sequence_change(self):
        ...
        """
        default_website_data = self.IrModelData.search(self.DefaultWebsiteDomain)
        initial_default_website = self.Website.search([('id', '=', default_website_data.res_id)])

        # Create new website
        new_website = self.Website.create({'name': 'New Website', 'sequence': initial_default_website.sequence + 1})

        # The initial website is still default
        self.assertEqual(default_website_data.res_id, initial_default_website.id)
    ...
    def test_04_ensure_default_moves_settings_when_needed(self):
        ...
        settings_to_be_transferred = [
            s for s in self.Website._get_settings_to_transfer_to_new_default_website() if s in self.Website._fields
        ]
