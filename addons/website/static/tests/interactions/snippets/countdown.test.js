...
test("Countdown snippet exists, when the colors are not defined", async () => {
    registry
        ...
        .add("test_snippets", function (namespace, snippets) {
            const countdownEl = snippets.querySelector("[data-snippet='s_countdown']");
...
test("past date: redirect end message is shown", async () => {
    addCountdownSnippetPreprocessor({ endAction: "redirect", endTime: 1 });
    await startInteractionsWithSnippet("s_countdown");
    await tick();
...
test("past date: end message is shown", async () => {
    addCountdownSnippetPreprocessor({ endAction: "message", endTime: 1 });
    await startInteractionsWithSnippet("s_countdown");
    await tick();
