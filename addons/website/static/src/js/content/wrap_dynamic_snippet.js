...
function _rewrapDynamicSnippet(el, isSmallDevice) {
    ...
    const rows = [...el.querySelectorAll("[data-fengshui-row-limit]")];
    const cols = [...el.querySelectorAll("[data-fengshui-row-limit] [data-fengshui-col]")];
    if (cols.length) {
        const rowPerSlide =
            parseInt(el.querySelector(".carousel[data-row-per-slide]")?.dataset.rowPerSlide) || 1;
        const chunkSize =
            computeChunkSize(rows[0].dataset, {
                limitName: "fengshuiRowLimit",
                numberOfElementsName: "fengshuiRowNumberOfElements",
                numberOfElementsSmallDevicesName: "fengshuiRowNumberOfElementsSmallDevices",
            }) * rowPerSlide;

        const colClass = `col-${Math.trunc((12 * rowPerSlide) / chunkSize)}`;
        for (const col of cols) {
            col.classList.remove(col.dataset.fengshuiCol);
            col.classList.add(colClass);
            col.dataset.fengshuiCol = colClass;
        }
        for (const row of rows) {
            const placeholder = document.createElement("div");
            placeholder.dataset.fengshuiCol = "";
            row.querySelector("[data-fengshui-col]").before(placeholder);
        }
        ...
        rows.forEach((row, index) => {
            row.querySelector("[data-fengshui-col]").parentElement.replaceChildren(
