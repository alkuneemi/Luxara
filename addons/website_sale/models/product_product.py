...
class ProductProduct(models.Model):
    ...
    def set_sequence_up(self):
        previous_variant = self.sudo().search(
            [
        ...
        if previous_variant:
            previous_variant.website_sequence, self.website_sequence = (
                self.website_sequence,
                previous_variant.website_sequence,
            )
        else:
            self.set_sequence_top()

    def set_sequence_down(self):
        next_variant = self.search(
            [
                ("website_sequence", ">", self.website_sequence),
                ("website_published", "=", self.website_published),
            ],
        ...
        if next_variant:
            next_variant.website_sequence, self.website_sequence = (
                self.website_sequence,
                next_variant.website_sequence,
            )
        else:
        ...
        for variant in self:
            ...
            if pricelist_item._show_discount_on_shop():
                pricelist_base_price = pricelist_item._compute_price_before_discount(
                    ...
                    quantity=1.0,
                    date=date,
                    uom=variant.product_tmpl_id._get_main_uom(),
                    currency=currency,
                )
