...
class Cart(PaymentPortal):
    ...
    def _get_express_shop_payment_values(self, order, **_kwargs):
        ...
        if provider_sudo:
            payment_form_values["provider_sudo"] = provider_sudo
            payment_form_values["payment_method_unknown_id"] = request.env.ref(
                f"payment_{provider_sudo.code}.payment_method_unknown"
            ).id
