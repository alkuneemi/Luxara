...
class TestGeneratorNullFracValidation(TransactionCase):
    ...
    def test_null_frac_on_required_field_raises(self):
        with self.assertRaises(ValueError) as cm:
            Char(field=self.name_field, env=self.env, length=10, null_frac=0.5)

        self.assertIn("required", str(cm.exception))

    def test_null_frac_with_weighted_values_raises(self):
        with self.assertRaises(ValueError) as cm:
            Sample(field=self.price_field, env=self.env, values={'a': 3, 'b': 1}, null_frac=0.2)
