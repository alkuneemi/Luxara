...
> **Note:** After installing a new module that ships blueprints, you must
> **upgrade the `populate` module** (e.g. `odoo-bin -d <database> -u populate`)
> so that its blueprints are discovered and loaded into the database.
...
| `null_frac`    |          | Probability of generating `False` (0-1, default `0`; forced to `0` for required fields) |
