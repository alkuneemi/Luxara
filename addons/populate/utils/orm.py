...
class DynamicDomain:
    ...
    **Example**:
    ...
    def __new__(cls, domain_expr: str) -> DynamicDomain | Domain:
        ...
        args = ', '.join(dynamic_fields)
        resolver = safe_eval(f'lambda {args}: {domain_expr}')

        def checked(**eval_kwargs):
            check_eval_kwargs(eval_kwargs)
            return resolver(**eval_kwargs)
...
def get_ref_domain(
    ...
    """Given a ref for a specific model, return a domain representing those records."""
