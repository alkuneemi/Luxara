...
class Distribution(ABC):
    ...
    def __eq__(self, other):
        ...
        return all(getattr(self, attr) == getattr(other, attr) for attr in vars(self) if attr != 'rng')

    def __hash__(self):
        return hash(tuple(sorted((k, v) for k, v in vars(self).items() if k != 'rng')))

    def __str__(self):
        params = ', '.join(f"{k}={v}" for k, v in sorted(vars(self).items()) if k != 'rng')
        if params:
            return f"{self.__class__.__name__}({params})"
        return f"{self.__class__.__name__}"

    def __repr__(self):
        params = ', '.join(f"{k}={v!r}" for k, v in sorted(vars(self).items()) if k != 'rng')
        if params:
