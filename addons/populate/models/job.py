...
class Job(models.Model):
    ...
    def _execute(self, generators: Mapping[str, Generator] | None = None, *, seed: int | None = None):
        ...
        with execution_scope(self):
            ...
            if generators is None:
                generators = self.__create_generators(seed)
    ...
    def __create_generators(self, seed: int | None = None) -> Mapping[str, Generator]:
        self.ensure_one()
        generators = {}
        model = self.env[self.model_name]
...
def execution_scope(job: Job):
    ...
    if not job.session_id.is_parallel or (job.is_executable and not job.parent_id):
        # This job committed in the same tx as the one that locked the session -> re-lock the session.
        job.session_id.lock_for_update()
