...
class Generator(GeneratorABC):
    ...
    :ivar allowed_fields_type: A tuple of allowed field types for this generator,
     or `None` for no restriction.
    """
    name: ClassVar[str]
    allowed_fields_type: ClassVar[tuple[str] | None] = None
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.field.required and null_frac:
            raise ValueError(self.env._(
        ...
        if self.has_weights and null_frac:
            # Don't allow null_frac with weighted values as it would throw off the requested bias
            raise ValueError(self.env._(
                "Cannot specify a null fraction when using weighted values. "
                "The 'null_frac' parameter is incompatible with custom weights.",
            ))
